...
export default function AnalyticsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Analytics"
        subtitle="Track orders and revenue trends over time"
        action={
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white">
          <p className="text-blue-200 text-sm font-medium">Total Orders ({days}d)</p>
          <p className="text-3xl font-bold mt-1">{totalOrders}</p>
        </div>
        <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl p-6 text-white">
          <p className="text-emerald-200 text-sm font-medium">Total Revenue ({days}d)</p>
      ...
      <div className="grid grid-cols-1 gap-6">
        <Card title="Orders Over Time">
          {ordersData.length === 0 ? (
            <p className="text-slate-500 text-sm text-center py-8">No data available for this period</p>
          ) : (

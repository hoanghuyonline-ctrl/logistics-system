...
/* ------------------------------------------------------------------ */
/* Dữ liệu mẫu khởi tạo ban đầu cho Trang Chủ                          */
/* ------------------------------------------------------------------ */
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-slate-400 bg-slate-50 rounded-lg px-3 py-2 border border-slate-100">
        💡 Dịch vụ sẽ hiển thị theo lưới tự động: 1 cột (mobile) → 2 cột (tablet) → 4 cột (desktop).
      </p>
    </div>
  );
}
...
export default function AdminHomePage() {
  ...
  const updateBlock = useCallback(
    (id: string, fields: Partial<HomePageBlock>) => {
      setBlocks((prev) =>
  ...
  /* Select a block and jump to content tab on mobile */
  const selectBlock = useCallback((id: string) => {
    setSelectedId(id);
    setActiveTab('content');
  }, []);

  /* Save handler (simulates DB persistence) */
  const handleSave = useCallback(() => {
    // In production this would be: await fetch('/api/admin/homepage', { method: 'PUT', body: JSON.stringify(blocks) });
    console.info('[CMS] Saving homepage config:', JSON.stringify(blocks, null, 2));
    setSaved(true);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Kéo thả để sắp xếp thứ tự khối · Nhấn vào khối để chỉnh nội dung
          </p>
        </div>
        <button
          id="cms-save-btn"
          onClick={handleSave}
          className={`inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold shadow-sm transition-all duration-200 ${
            saved
        ...
        >
          <div className="border-b border-slate-100 px-5 py-4">
            ...
            <p className="mt-0.5 text-xs text-slate-400">
              Kéo thả để thay đổi thứ tự · Tick ô để ẩn/hiện khối
            </p>
          </div>
          <div className="p-4">
                      ...
                      >
                        {(draggableProvided, snapshot) => (
                          <div
                            ...
                            /* touch-none prevents scroll from stealing the drag gesture */
                            className={`flex cursor-pointer touch-none items-center justify-between gap-3 rounded-xl border-2 p-4 transition-all duration-150 ${
                              snapshot.isDragging
        ...
        >
          <div className="border-b border-slate-100 px-5 py-4">
            ...
            {selectedBlock ? (
              ...
              <p className="mt-0.5 text-xs text-slate-400">
                Chọn một khối bên trái để bắt đầu chỉnh sửa
              </p>
            )}
          </div>
